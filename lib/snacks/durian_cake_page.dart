...
void main() {
  runApp(const MaterialApp(home: DurianCakePage()));
}

class DurianCakePage extends StatefulWidget {
  const DurianCakePage({super.key});
...
class _DurianCakePageState extends State<DurianCakePage> {
  ...
  final int price = 250; // ราคาต่อชิ้น
  ...
  void initState() {
    super.initState();
    const videoUrl = 'https://youtu.be/J7OLGiw47Qw';
  ...
  void _addToCart() {
    ...
    Cart.addItem("เค้กทุเรียน${comment.isNotEmpty ? ' ($comment)' : ''}", price, qty);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("เค้กทุเรียน"), backgroundColor: Colors.deepOrange),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ExpansionTile(
            ...
            children: const [
              Padding(
                padding: EdgeInsets.all(8.0),
                child: Text("1. ผสมแป้ง ไข่ น้ำตาล\n2. ใส่เนื้อทุเรียน\n3. อบจนสุก"),
              ),
            ],
          ),
