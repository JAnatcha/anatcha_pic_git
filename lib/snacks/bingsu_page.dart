...
void main() {
  runApp(const MaterialApp(home: BingsuPage()));
}

class BingsuPage extends StatefulWidget {
  const BingsuPage({super.key});
...
class _BingsuPageState extends State<BingsuPage> {
  ...
  final int price = 120; // ราคาต่อถ้วย
  ...
  void initState() {
    super.initState();
    const videoUrl = 'https://youtu.be/kxlCi36ubyA';
  ...
  void _addToCart() {
    ...
    Cart.addItem("บิงซูสตรอว์เบอร์รี่${comment.isNotEmpty ? ' ($comment)' : ''}", price, qty);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("บิงซูสตรอว์เบอร์รี่"), backgroundColor: Colors.deepOrange),
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text("ราคา $price บาท / ถ้วย"),
          const SizedBox(height: 6),
          TextField(controller: qtyCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(hintText: "จำนวน")),
          const SizedBox(height: 6),
          TextField(controller: commentCtrl, decoration: const InputDecoration(hintText: "หมายเหตุ เช่น ลดหวาน")),
          const SizedBox(height: 10),
